...
docker_run="docker run -d --env AWS_ACCESS_KEY_ID='${AWS_ACCESS_KEY_ID}' --env AWS_SECRET_ACCESS_KEY='${AWS_SECRET_ACCESS_KEY}' --env AWS_SESSION_TOKEN='${AWS_SESSION_TOKEN}' -v ${config_fileloc}:/home/glue/.aws -v ${nb_fileloc}:/home/glue/notebook -p 8000:8000 --rm --name jupyter_notebook ${container} ${docker_cmd}"
